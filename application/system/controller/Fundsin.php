...
use app\system\model\Fundsins;

class Fundsin extends Auth
{
    /* ========== 初始化 ========== */
    ...
    public function index()
    {
        ...
        if ($entry_at) {
            $entry_at_start = strtotime($entry_at." 00:00:00");
            $entry_at_end = strtotime($entry_at." 23:59:59");
            $where['f.entry_at'] = [['<=',$entry_at_end],['>=',$entry_at_start]];
            $datas['entry_at'] = $entry_at;
        }
        /* ++++++++++ 排序 ++++++++++ */
        ...
        $fundsin_model=new Fundsins();
        if(is_numeric($deleted) && in_array($deleted,[0,1])){
            $datas['deleted']=$deleted;
            if($deleted==1){
                $fundsin_model=$fundsin_model->onlyTrashed();
            }
        }else{
            $fundsin_model=$fundsin_model->withTrashed();
        }
        $fundsin_list=$fundsin_model
            ->alias('f')
            ->field($field)
        ...
        $datas['fundsin_list']=$fundsin_list;
    ...
    public function add(){
        if (request()->isPost()) {
            ...
            $rule = [
                ...
                ['entry_at', 'require', '请选择缴纳时间'],
                ['payer', 'require', '请输入缴纳人'],
                ['amount', 'require', '请输入金额'],
                ['bank', 'require', '请输入支付银行'],
    ...
    public function edit(){
        ...
        $rs = model('Fundsins')->save([
                'voucher'=>$datas['voucher'],
                'entry_at'=>$datas['entry_at'],
                'payer'=>$datas['payer'],
                'amount'=>$datas['amount'],
                'bank'=>$datas['bank'],
    ...
    public function delete(){
        ...
        $res = model('Fundsins')->destroy(['id'=>['in',$ids]]);
        if($res){
    ...
    public function restore(){
        ...
        $res = db('funds_in')->whereIn('id',$ids)->update(['deleted_at'=>null,'updated_at'=>time()]);
        if($res){
    ...
    public function destroy(){
        ...
        $res = model('Fundsins')->onlyTrashed()->whereIn('id',$ids)->delete(true);
        if($res){
